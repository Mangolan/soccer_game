...
- 사용 도구: OpenAI Codex
- 활용 내역: 게임 기능과 적응형 AI 구현 보조, 코드 오류 분석 및 수정, 테스트 코드 작성,
  Android APK 빌드·GitHub Release 자동화 설정, README 및 기술 문서 작성·검토
- 검토 방식: AI가 제안하거나 수정한 코드와 문서는 개발자가 직접 검토했으며, 실행 및
  테스트를 거쳐 프로젝트에 반영했습니다.

게임에 포함된 적응형 AI는 생성형 AI 모델이나 외부 AI API를 호출하지 않는 휴리스틱
기반 로컬 게임 로직입니다. 플레이 데이터는 외부 서비스로 전송되지 않습니다.
